...
class XChinaExtractor(Extractor):
    ...
    def items(self):
        try:
            ...
            for page_num in range(1, total_pages + 1):
                ...
                # 从当前页面提取图片
                image_containers = soup.select(".photo-image")
    ...
    def _request_page(self, page_num):
        """请求相册的特定页面"""
        if page_num == 1:
            url = f"{self.root}/photo/id-{self.album_id}.html"
        else:
            url = f"{self.root}/photo/id-{self.album_id}/{page_num}.html"
...
class XChinaPhotoExtractor(XChinaExtractor):
    """Extractor for amateur galleries from xchina.co"""
    subcategory = "photo"
    pattern = r"(?:https?://)?(?:www\.)?xchina\.fit/photo/id-([a-z0-9]+)(?:/\d+)?.html"
    example = "https://xchina.fit/photo/id-670669c311474.html"
